...
The outer surface of the ship is usually not uniform as not all plates have the same thickness, Therefore the hull surface is measured with respect to the inner surface of the plating which is termed as \emph{moulded surface} of the hull. All dimensions measured to this surface is defined as \emph{moulded} dimensions whereas dimensions measured to the outer surface of the hull or of an appendage are qualified as \emph{extreme} \bcitep{Biran.2014}.

\subsubsection*{Coefficients of form}

The form coefficients are non-dimensional numbers required to classify the hulls and to find relationships between forms and their properties, the summary of some importan form coefficients are summarised in \Cref{fig:man_formcoeff} 
...
\textbf{Block Coefficient $C_B$} is defined as the ratio of moulded displacement volume to the volume of parallelepiped (rectangular block) with dimension $L,B$ and $T$. Alternatively, \bcitet{Schneekluth.1998} provided and estimation of the value using the Froude number within range of $0.15 < Fr < 0.32$
...
The relative rotative efficiency $\eta_R$ can be expressed by the following ratio, with $V_A$ defined as the arriving water velocity to propeller \bcitep{Diesel.2011}:
...
According to \bcitet{Holtrop.1982}, $\eta_R$ for twin screw vessels can be estimated using the following formula, with $\frac{P}{D}$ defined as the propeller pitch to diameter ratio:
