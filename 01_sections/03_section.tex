...
The features \begin{enumerate*}
    \item wind direction,
    \item swell direction,
    \item wind wave direction
...
This conversion is performed as the information of current speed and current direction, $\gamma$, is necessary to perform the correction formula shown in \Cref{eqn:stwx} and \Cref{eqn:stwy}. However, for training purpose, this feature will not be considered. Instead, the true current direction and true wind direction will be considered. The initial structure have 27 features, 9 AIS features and 18 weather features. The structure of the initial dataset i.e. before data preprocessing and feature selection, is summarised in \Cref{dataset_init_struct} \\
...
The imputing approach using k-nearest neighbour is also applied to other weather features that contained missing values i.e. {\tt NaN} values. Imputing missing values is necessary as modelling package by {\tt Scikit-learn} cannot handle missing values. Imputing strategy using k-nearest neighbour is considered as it reflected the weather conditions within the region of missing values.\\
...
From AIS data, the information on \begin{enumerate*}
    \item time
    \item latitude
    \item longitude
    \item width 
    \item length
\end{enumerate*} are not included for training. As time, latitude and longitude have no impact on the ship. While the width and length is constant.\\  
...
For training, the dataset is split into training, validation and test dataset in ratio of 73:18:9. Journey data from the month of June is arbitrarily selected as test dataset. The remaining dataset will be split into training and validation dataset in 80:20 ratio. 
